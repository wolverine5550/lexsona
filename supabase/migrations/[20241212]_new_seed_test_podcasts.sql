...
DELETE FROM public.podcast_analysis WHERE podcast_id LIKE 'test%';
DELETE FROM public.podcasts WHERE id LIKE 'test%';
...
('test1', 'Tech Innovators', 'Deep dives into technology innovation and startups', 'Tech Media', 
    '[67, 127]'::jsonb, 100, 80, 'en', false, extract(epoch from now()) * 1000, 'https://techinnovators.example.com', now(), now()),
('test2', 'AI Revolution Weekly', 'Latest developments in artificial intelligence and machine learning', 'Future Labs', 
    '[125, 67]'::jsonb, 75, 85, 'en', false, extract(epoch from now()) * 1000, 'https://airevolution.example.com', now(), now()),
('test3', 'Startup Success Stories', 'Interviews with successful entrepreneurs and business leaders', 'Founder Media', 
    '[93, 95]'::jsonb, 50, 75, 'en', false, extract(epoch from now()) * 1000, 'https://startupstories.example.com', now(), now()),
('test4', 'Tech Leadership Today', 'Insights from technology leaders and innovators', 'Tech Insights', 
    '[67, 122]'::jsonb, 120, 70, 'en', false, extract(epoch from now()) * 1000, 'https://techleadership.example.com', now(), now()),
('test5', 'Future of AI', 'Exploring the future of artificial intelligence and its impact', 'AI Research', 
    '[125, 111]'::jsonb, 60, 90, 'en', false, extract(epoch from now()) * 1000, 'https://futureofai.example.com', now(), now());
